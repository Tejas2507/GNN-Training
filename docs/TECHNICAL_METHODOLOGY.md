...
*   **Convolution Layer (`MySAGEConv`)**:
    ...
    $$\mathbf{h}\sb{i}^{(l+1)} = \mathbf{W}\sb{1} \cdot \text{AGGREGATE}\left(\{\mathbf{h}\sb{j}^{(l)}, \forall j \in \mathcal{N}(i)\}\right) + \mathbf{W}\sb{2} \cdot \mathbf{h}\sb{i}^{(l)}$$
...
*   **Pooling Layer (`pooling`)**: Aggregates neighbor representations using non-parametric mean pooling followed by a linear projection:
    ...
    $$\mathbf{z}\sb{i}' = \text{Linear}(\text{mean-pool}(\mathbf{z}\sb{i}, \mathcal{N}(i)))$$
...
$$\mathcal{L}\sb{\text{total}} = \lambda\sb{\text{feat}} \mathcal{L}\sb{\text{feat}} + \lambda\sb{\text{topo}} \mathcal{L}\sb{\text{topo}} + \lambda\sb{\text{sem}} \mathcal{L}\sb{\text{sem}} + \lambda\sb{\text{align}} \mathcal{L}\sb{\text{align}}$$
...
1.  **Feature Reconstruction Loss ($\mathcal{L}_{\text{feat}}$)**: Computes the MSE between the decoded representations and the original semantic text embeddings:
    ...
    $$\mathcal{L}\sb{\text{feat}} = \frac{1}{|B|} \sum\sb{i \in B} \|\text{Decoder}(\mathbf{z}\sb{i}) - \mathbf{x}\sb{i}\|^2$$
...
2.  **Topological Reconstruction Loss ($\mathcal{L}_{\text{topo}}$)**: Optimizes binary cross-entropy on positive edges and negative sampled node pairs using an inner-product decoder:
    ...
    $$\mathcal{L}\sb{\text{topo}} = -\sum\sb{(u,v) \in E^{+}} \log(\sigma(\mathbf{z}\sb{u}^\top \mathbf{z}\sb{v})) - \sum\sb{(u,w) \in E^{-}} \log(1 - \sigma(\mathbf{z}\sb{u}^\top \mathbf{z}\sb{w}))$$
...
3.  **Semantic Contrastive Loss ($\mathcal{L}_{\text{sem}}$)**: Measures cosine similarity between the online GNN representations ($\mathbf{h}_1$, $\mathbf{h}_2$) and target representations ($\mathbf{z}_1$, $\mathbf{z}_2$) generated by an EMA-updated encoder ($\theta_{\text{EMA}} \leftarrow \alpha \theta_{\text{EMA}} + (1-\alpha)\theta_{\text{online}}$):
    ...
    $$\mathcal{L}\sb{\text{sem}} = \frac{1}{2} \left[ (1 - \text{cosine-sim}(\mathbf{z}\sb{1}, \mathbf{h}\sb{2})) + (1 - \text{cosine-sim}(\mathbf{z}\sb{2}, \mathbf{h}\sb{1})) \right]$$
...
4.  **Alignment Regularization ($\mathcal{L}_{\text{align}}$)**: Minimizes representation collapse by calculating the KL divergence between the average batch distribution and a uniform target distribution:
    ...
    $$\mathcal{L}\sb{\text{align}} = \text{KL}\left(\text{Softmax}(\mathbf{z}\sb{i}) \parallel \text{Softmax}(\bar{\mathbf{z}})\right) \cdot \lambda\sb{\text{align}}$$
...
2.  **Classifier Head Construction**: Instantiates a downstream model wrapper (`TaskModel`) combining the GNN encoder and a linear decoder:
    ...
    $$\mathbf{z}\sb{i} = \text{Encoder}(\mathbf{x}\sb{i}, \mathbf{A})$$
    
    $$\mathbf{z}\sb{i}' = \text{pooling-lin}(\text{mean-aggregate}(\mathbf{z}\sb{i}))$$
    
    $$\mathbf{logits}\sb{i} = \mathbf{W}\sb{\mathrm{clf}} \cdot \mathbf{z}\sb{i}' + \mathbf{b}$$
...
*   **Accuracy**:
    ...
    $$\text{Accuracy} = \frac{1}{|T\sb{\text{test}}|} \sum\sb{i \in T\sb{\text{test}}} \mathbb{I}(\hat{y}\sb{i} = y\sb{i})$$
    
*   **Precision (Macro)**:
    
    $$\text{Precision}\sb{\text{macro}} = \frac{1}{C} \sum\sb{c=1}^C \frac{TP\sb{c}}{TP\sb{c} + FP\sb{c}}$$
    
*   **Recall (Macro)**:
    
    $$\text{Recall}\sb{\text{macro}} = \frac{1}{C} \sum\sb{c=1}^C \frac{TP\sb{c}}{TP\sb{c} + FN\sb{c}}$$
    
*   **F1-Score (Macro)**:
    
    $$\text{F1}\sb{\text{macro}} = \frac{1}{C} \sum\sb{c=1}^C 2 \cdot \frac{\text{Precision}\sb{c} \cdot \text{Recall}\sb{c}}{\text{Precision}\sb{c} + \text{Recall}\sb{c}}$$
...
*   **Average Precision (Binary)**: Calculates the Precision-Recall Area Under the Curve (PR-AUC):
    ...
    $$\text{AP} = \sum\sb{n} (R\sb{n} - R\sb{n-1}) P\sb{n}$$
